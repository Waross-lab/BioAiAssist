...
import { z } from 'zod';
import { ResearchRun, researchRun, ResearchRunT } from './research_orchestrator.js';
...
const qtile = (xs:number[], q:number) => {
  if (!xs.length) return undefined;
  const a=[...xs].sort((x,y)=>x-y); const i=Math.max(0,Math.min(a.length-1,Math.floor(q*(a.length-1))));
  return a[i];
};
...
export type HypothesisRunT = z.infer<typeof HypothesisRun>;
...
async function discoverTargets(tokens: string[], limit=8): Promise<{query:string, label:string}[]> {
  ...
  })).filter((x:any)=>x.query).slice(0,limit);
  return arr;
}

// Discovery: try PubChem for compounds from tokens (best-effort)
async function discoverCompounds(tokens: string[], limit=6): Promise<{name:string}[]> {
  ...
  for (const t of tokens.slice(0,12)) {
    ...
    if (j?.IdentifierList?.CID?.length) out.push({name:t});
...
function detectContextCues(h: string) {
  const H = h.toLowerCase();
  const organisms: string[] = [];
  if (/\bhuman\b|homo sapiens/i.test(H)) organisms.push('Homo sapiens');
  if (/\bmouse\b|mus musculus/i.test(H)) organisms.push('Mus musculus');
  if (/\brate\b|rattus norvegicus/i.test(H)) organisms.push('Rattus norvegicus');
  const binomial = /([A-Z][a-z]+)\s([a-z]{3,})/g; // crude binomial detector
  let m; while ((m=binomial.exec(h))!==null) { organisms.push(`${m[1]} ${m[2]}`); }

  const techniques: string[] = [];
  if (/TRAP assay|telomerase repeat amplification/i.test(h)) techniques.push('TRAP');
  if (/\bMIC\b/i.test(h)) techniques.push('MIC');
  if (/\bqPCR\b/i.test(h)) techniques.push('qPCR');
  if (/western blot/i.test(h)) techniques.push('Western blot');
  if (/ELISA/i.test(h)) techniques.push('ELISA');
  if (/CRISPR/i.test(h)) techniques.push('CRISPR');
...
function booleanQuery(compoundNames: string[], keywords: string[], organisms: string[]) {
...
async function autoscope(hypothesis: string, spec: ResearchRunT): Promise<ResearchRunT> {
  // 1) Tokenize & seed keywords
  const tokens = tokenize(hypothesis);
  const kws = new Set<string>(tokens.slice(0,12));

  // 2) Context cues → adjust options generically
  const cues = detectContextCues(hypothesis);
  const options = {...(spec.options||{})};
  if (!options.organism_contains && cues.organisms[0]) options.organism_contains = cues.organisms[0];
  // Activation/agonism → allow non-pChEMBL; inhibition often has pChEMBL
  if (cues.intents.activation && options.pchemblOnly !== false) options.pchemblOnly = false;
  ...
  for (const t of tHits) if (t.label) kws.add(t.label);
  for (const c of cHits) if (c.name) kws.add(c.name);
  cues.techniques.forEach(t => kws.add(t));
  if (cues.intents.activation) kws.add('activation');
  if (cues.intents.inhibition) kws.add('inhibition');
 ...
 * ---------------------------- */
function analyzeGeneric(normalized: AnyObj) {
  ...
  for (const t of T) { const org = s(t.organism || t.organism_name); if (org) byOrg[org]=(byOrg[org]||0)+1; }
  const topOrganisms = Object.entries(byOrg).map(([organism,count])=>({organism,count})).sort((a,b)=>b.count-a.count).slice(0,8);

  // Compound physchem
  const pick=(k:string)=> C.map((c:any)=>num(c[k]??c[k?.toUpperCase?.()]??c[k?.toLowerCase?.()])).filter((x:any)=>x!=null) as number[];
  ...
  const topLit = [...L].sort((a:any,b:any)=>{
    ...
    return (bd-ad)||(by-ay);
  }).slice(0,10).map((l:any)=>({title:s(l.title||l.display_name), year:s(l.year||l.pubYear||l.publication_year), doi:s(l.doi), pmid:s(l.pmid)}));

  return { targetStats, topOrganisms, compSummary, topLit };
}

// Technique-specific sidebars (activated only if cues found)
function analyzeTechniques(normalized: AnyObj, cues: {techniques:string[]}) {
  ...
  if (cues.techniques.includes('TRAP')) {
    ...
    const trapL = L.filter((x:any)=> trapRx.test(s(x.title)));
    out.trap = { mentions: trapL.length, examples: trapL.slice(0,5).map((l:any)=>({title:s(l.title), year:s(l.year||l.pubYear||l.publication_year), doi:s(l.doi)})) };
  }
  if (cues.techniques.includes('MIC')) {
    const micA = A.filter((a:any)=> /MIC/i.test(s(a.standard_type)));
    const vals = micA.map((a:any)=> num(a.standard_value)).filter((x:any)=>x!=null) as number[];
 ...
 * ---------------------------- */
function renderMD(hyp: string, metrics: AnyObj, core: AnyObj, tech: AnyObj) {
  const m=metrics||{};
...
${(tech.trap.examples||[]).map((e:any)=>`- ${e.year||'n.d.'} — ${e.title}${e.doi?` (DOI: ${e.doi})`:''}`).join('\n')}\n\n`;
  }
  if (tech.mic) {
...
${core.targetStats.map((t:any)=>`- ${t.target_id}: mean pChEMBL ${t.pchembl_mean?.toFixed(2) ?? 'n/a'} (n=${t.n})`).join('\n') || '- n/a'}

### Organisms (Top)
${core.topOrganisms.map((o:any)=>`- ${o.organism}: ${o.count}`).join('\n') || '- n/a'}
...
${core.topLit.map((l:any)=>`- ${l.year||'n.d.'} — ${l.title}${l.doi?` (DOI: ${l.doi})`:''}${l.pmid?` [PMID:${l.pmid}]`:''}`).join('\n') || '- n/a'}
 ...
 * 4) Main entry
 * ---------------------------- */
export async function hypothesisRun(input: HypothesisRunT) {
  // Build a scoped spec from the hypothesis (generic, no hardcoded biology)
  const baseSpec: ResearchRunT = ResearchRun.parse(input.research);
