...
export type ResearchRunT = z.infer<typeof ResearchRun>;

// ---------- Helpers for sources ----------

// PubChem: very small helper to fetch props by compound name (best-effort)
async function pubchemPropsByName(name: string): Promise<any[]> {
...
async function chemblTargetSearch(q: string, limit = 25, organism?: string, nameContains?: string): Promise<any[]> {
  const params = new URLSearchParams({ format: 'json', limit: String(limit) });
  if (organism) params.set('organism__icontains', organism);
  // Prefer explicit query; otherwise fall back to name_contains if given
  if (q) params.set('target_synonym__icontains', q);
  if (nameContains) params.set('pref_name__icontains', nameContains);
  try {
...
async function chemblActivitiesByTarget(targetChemblId: string, limit = 50, pchemblOnly = true): Promise<any[]> {
  const params = new URLSearchParams({ format: 'json', limit: String(limit), target_chembl_id: targetChemblId });
  if (pchemblOnly) params.set('pchembl_value__isnull', 'false');
  try {
...
function buildUniProtQuery(input: string | string[], opts?: { organism?: string; reviewed?: boolean }) {
  const toTok = (t: string) => (/\s/.test(t) ? `"${t}"` : t);
  const base = Array.isArray(input) ? `(${input.filter(Boolean).map(toTok).join(' OR ')})` : String(input || '').trim();
  const clauses: string[] = [];
  if (base) clauses.push(base);
  if (opts?.reviewed !== false) clauses.push('reviewed:true');
  if (opts?.organism) clauses.push(`organism_name:"${opts.organism}"`);
  return clauses.length ? clauses.join(' AND ') : 'reviewed:true';
}
async function uniprotSearch(query: string | string[], limit = 25, opts?: { organism?: string; reviewed?: boolean }): Promise<any> {
...
function booleanFromKeywords(keywords: string[], organisms: string[]) {
...
async function entrezEsearch(q: string, retmax = 50): Promise<{count:number, ids:string[]}> {
  try {
...
async function europepmcSearch(q: string, pageSize = 50): Promise<any[]> {
  try {
...
async function openalexWorks(q: string, perPage = 50): Promise<any[]> {
  try {
...
function summarizeNormalization(normalized: AnyObj) {
  ...
  const T = (normalized as any).targets || [];
  const A = (normalized as any).assays || [];
  const L = (normalized as any).literature || [];

  const pct = (num: number, den: number) => den ? (100 * num / den) : 0;

  // Compounds
  const c_inchikey = C.filter((c: any) => c.InChIKey || c.inchikey).length;
  const c_props = C.filter((c: any) =>
    c.MolecularWeight != null || c.MolecularFormula || c.CanonicalSMILES || c.canonicalSmiles
  ).length;
  const inchikeys = C.map((c:any)=> (c.InChIKey || c.inchikey || '').toString()).filter(Boolean);
  ...
  const a_std_num = A.filter((a:any)=> {
    const v = Number(a.standard_value); return Number.isFinite(v);
  }).length;
  const targetIds = new Set(T.map((t:any)=> String(t.target_chembl_id || t.chembl_target_id || t.uniprot || t.uniprot_id || '')).filter(Boolean));
  const a_linked = A.filter((a:any)=> {
    const id = String(a.target_chembl_id || a.target_id || a.uniprot || '');
    return id && targetIds.has(id);
  }).length;

  // Literature
  const Lkey = (l:any)=> (l.doi ? `doi:${l.doi}` : '') + '|' + (l.pmid ? `pmid:${l.pmid}` : '') + '|' + (l.title || '');
  const uniqLit = new Set(L.map(Lkey));
  const l_pmid = L.filter((l:any)=> !!l.pmid).length;
  const l_doi  = L.filter((l:any)=> !!l.doi).length;
...
// ---------- Orchestration ----------
export async function researchRun(p_in: ResearchRunT) {
  ...
  const term =
    (p.keywords && p.keywords.length ? p.keywords.join(' ') : '') ||
  ...
  // -------- PubChem props --------
  let pubchem_props: any[] = [];
  if (p.sources.includes('pubchem')) {
    const names = Array.from(new Set(p.compounds.map(c => c.name).filter(Boolean) as string[])).slice(0, p.options.maxPerSource);
  ...
  let chembl_targets: any[] = [];
  let chembl_activities: any[] = [];
  if (p.sources.includes('chembl')) {
    const queries = new Set<string>();
    for (const t of p.targets) {
      if (t.query) queries.add(t.query);
      if (t.symbol) queries.add(t.symbol);
    }
    if (!queries.size && term) queries.add(term);
  ...
  let uniprot_results: any = { results: [] };
  if (p.sources.includes('uniprot')) {
    const tokens = Array.from(new Set([
      ...p.keywords,
      ...p.targets.map(t => (t.query || t.symbol)).filter(Boolean) as string[],
    ])).slice(0, 12);
  ...
  let pubmed_esearch: any = { count: 0, ids: [] };
  let europepmc_search: any[] = [];
  let openalex_works: any[] = [];
  ...
  // ----- Final organism gating before normalization -----
  {
    ...
    const _incl = (s: string) => !_org || (s || '').toLowerCase().includes(_org);

    chembl_targets = (chembl_targets || []).filter((t: any) => _incl(String(t.organism || '')));

    if (uniprot_results && Array.isArray((uniprot_results as any).results)) {
      (uniprot_results as any).results = (uniprot_results as any).results.filter((r: any) => {
  ...
  // -------- Normalize & summarize --------
  const normInput: z.infer<typeof NormalizeEntitiesInput> = {
    pubchem_props,
    chembl_targets,
    chembl_activities,
    uniprot_results,
