...
function cov(x, y, mx = mean(x), my = mean(y)) {
    const n = x.length;
    let s = 0;
    for (let i = 0; i < n; i++)
        s += (x[i] - mx) * (y[i] - my);
...
function pearsonR(x, y) {
    ...
    if (x.length < 3)
        throw new Error('need at least 3 points for Pearson r');
    const sx = stddev(x), sy = stddev(y);
    if (sx === 0 || sy === 0)
        return { r: 0, undefinedVariance: true };
...
tools.push({
    ...
    handler: async (input) => {
        ...
        if (p.entrez_query)
            params.set('ENTREZ_QUERY', p.entrez_query);
        ...
        if (!rid)
            throw new Error('BLAST submit failed: no RID in response');
...
tools.push({
    ...
    handler: async (input) => {
        ...
        const txt = await r.text();
        if (/Status=WAITING/.test(txt))
            return { status: 'WAITING' };
        if (/Status=FAILED/.test(txt))
            return { status: 'FAILED' };
        if (/Status=UNKNOWN/.test(txt))
            return { status: 'UNKNOWN' };
        if (/Status=READY/.test(txt))
            return { status: 'READY', has_hits: /ThereAreHits=yes/.test(txt) };
...
function summarizeBlastJson(json, top_n = 15) {
    try {
        ...
        for (const r of rows) {
            const key = r.taxon;
            if (!byTaxon[key])
                byTaxon[key] = { count: 0, best: r };
            byTaxon[key].count += 1;
            if ((r.identityP ?? 0) > (byTaxon[key].best.identityP ?? 0))
                byTaxon[key].best = r;
        }
...
tools.push({
    ...
    handler: async (input) => {
        ...
        if (!json)
            throw new Error('Provide either "rid" or "json".');
...
tools.push({
    ...
    handler: async (input) => {
        ...
        if (!json && p.rid) {
            ...
            const ct = r.headers.get('content-type') || '';
            if (!ct.includes('json'))
                return { error: 'Non-JSON response from NCBI' };
            json = await r.json();
        }
        if (!json)
            throw new Error('Provide either "rid" or "json".');
        const hits = json?.BlastOutput2?.[0]?.report?.results?.search?.hits || [];
        let rows = hits.map((h) => hitRow(h));
        if (p.organism_filter)
            rows = rows.filter((r) => (r.sciname || '') === p.organism_filter);
        const out = rows
            .sort((a, b) => (a.evalue ?? 1) - (b.evalue ?? 1) ||
...
tools.push({
    ...
    handler: async (input) => {
        ...
        if (!r.ok)
            throw new Error(`Ensembl overlap error ${r.status}`);
        return await r.json();
    }
});
tools.push({
    ...
    handler: async (input) => {
        ...
        const r = await fetch(url, { headers: headers() });
        if (!r.ok)
            throw new Error(`Ensembl xrefs error ${r.status}`);
        return await r.json();
    }
});
...
tools.push({
    ...
    handler: async (input) => {
        ...
        const r = await fetch(url.toString(), { headers: headers() });
        if (!r.ok)
            throw new Error(`EuropePMC error ${r.status}`);
        return await r.json();
    }
});
tools.push({
    ...
    handler: async (input) => {
        ...
        if (p.search)
            url.searchParams.set('search', p.search);
        if (p.filter)
            url.searchParams.set('filter', p.filter);
        url.searchParams.set('per-page', String(p.per_page));
        const r = await fetch(url.toString(), { headers: headers() });
        if (!r.ok)
            throw new Error(`OpenAlex error ${r.status}`);
        return await r.json();
    }
});
tools.push({
    ...
    handler: async (input) => {
        ...
        if (!r.ok)
            throw new Error(`Crossref error ${r.status}`);
        return await r.json();
    }
});
...
tools.push({
    ...
    handler: async (input) => {
        ...
        const r = await fetch(url.toString(), { headers: headers() });
        if (!r.ok)
            throw new Error(`Esearch error ${r.status}`);
        return await r.json();
    }
});
tools.push({
...
async function efetchNuccoreFastaSeq(idOrAcc) {
    const url = new URL('https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi');
...
tools.push({
    ...
    handler: async (input) => {
        ...
        if (!ct.includes('json'))
            return { error: 'Non-JSON response from NCBI' };
        const json = await r.json(); // typed
        const hits = json?.BlastOutput2?.[0]?.report?.results?.search?.hits || [];
        let rows = hits.map((hit) => {
            const d = hit?.description?.[0] || {};
            const h = hit?.hsps?.[0] || {};
        ...
        for (const row of rows) {
            ...
            if (!acc) {
                out.push({ error: 'missing accession', row });
                continue;
            }
            try {
...
tools.push({
    ...
    handler: async (input) => {
        ...
        for (const q of p.queries) {
            ...
            url.searchParams.set('format', 'json');
            const r = await fetch(url.toString(), { headers: headers() });
            const js = await r.json(); // typed
            const docs = js?.resultList?.result || [];
            all.push({
                query: q,
                results: docs.map((d) => ({
                    title: d.title,
...
tools.push({
    ...
    handler: async (input) => {
        ...
        url.searchParams.set('format', 'json');
        if (p.fields)
            url.searchParams.set('fields', p.fields);
        const r = await fetch(url.toString(), { headers: headers() });
        if (!r.ok)
            throw new Error(`UniProt search error ${r.status}`);
        return await r.json();
    }
});
tools.push({
    ...
    handler: async (input) => {
        ...
        const r = await fetch(url.toString(), { headers: headers() });
        if (!r.ok)
            throw new Error(`UniProt get error ${r.status}`);
        return await r.json();
    }
});
...
tools.push({
    ...
    handler: async (input) => {
        ...
        if (!DRUGBANK_API_KEY)
            throw new Error('Set DRUGBANK_API_KEY in environment to use DrugBank.');
...
tools.push({
    ...
    handler: async (input) => {
        ...
        const r = await fetch(url, { headers: headers() });
        if (!r.ok)
            throw new Error(`PubChem search error ${r.status}`);
        const j = await jsonAny(r);
        const ids = (j?.IdentifierList?.CID ?? []).map((x) => String(x));
        ...
        if (ids.length > 0) {
            ...
            if (pr.ok) {
                const pj = await jsonAny(pr);
                props = (pj?.PropertyTable?.Properties ?? []);
            }
        }
        return {
...
tools.push({
    ...
    handler: async (input) => {
        ...
        const r = await fetch(url, { headers: headers() });
        if (!r.ok)
            throw new Error(`PubChem props error ${r.status}`);
        const js = await jsonAny(r);
...
tools.push({
    ...
    handler: async (input) => {
        ...
        const r = await fetch(url, { headers: headers() });
        if (!r.ok)
            throw new Error(`PubChem synonyms error ${r.status}`);
        const js = await jsonAny(r);
...
tools.push({
    ...
    handler: async (input) => {
        ...
        const r = await fetch(url.toString(), { headers: headers() });
        if (!r.ok)
            throw new Error(`ChEMBL molecule search error ${r.status}`);
        const js = await jsonAny(r);
        return js?.molecules ?? [];
    }
});
tools.push({
    ...
    handler: async (input) => {
        ...
        url.searchParams.set('limit', String(p.limit));
        url.searchParams.set('offset', String(p.offset));
        if (p.organism_contains)
            url.searchParams.set('organism__icontains', p.organism_contains);
        if (p.name_contains)
            url.searchParams.set('pref_name__icontains', p.name_contains);
        const r = await fetch(url.toString(), { headers: headers() });
        if (!r.ok)
            throw new Error(`ChEMBL target search error ${r.status}`);
        const js = await jsonAny(r);
        return js?.targets ?? [];
    }
});
tools.push({
    ...
    handler: async (input) => {
        ...
        if (p.target_chembl_id)
            url.searchParams.set('target_chembl_id', p.target_chembl_id);
        if (p.molecule_chembl_id)
            url.searchParams.set('molecule_chembl_id', p.molecule_chembl_id);
        if (p.pchembl_only)
            url.searchParams.set('pchembl_value__isnull', 'false');
        url.searchParams.set('limit', String(p.limit));
        url.searchParams.set('offset', String(p.offset));
        const r = await fetch(url.toString(), { headers: headers() });
        if (!r.ok)
            throw new Error(`ChEMBL activities error ${r.status}`);
        const js = await jsonAny(r);
        // Return just a tidy subset of fields commonly used
        const rows = (js?.activities ?? []).map((a) => ({
...
tools.push({
    ...
    handler: async (input) => {
        ...
        if (p.standard_type)
            params.set('standard_type', p.standard_type);
        const url = `${base}?${params.toString()}`;
        const r = await fetch(url, { headers: headers() });
        if (!r.ok)
            throw new Error(`ChEMBL by_assay_query error ${r.status}`);
...
tools.push({
    ...
    handler: async (input) => {
        ...
        const r = await fetch(url, { headers: headers() });
        if (!r.ok)
            throw new Error(`ChEMBL by_molecule error ${r.status}`);
        const js = await r.json().catch(() => ({}));
        return { activities: js?.activities ?? [] };
    }
});
...
tools.push({
    ...
    handler: async (input) => {
        ...
        try {
            ...
            const opts = p.research.options = p.research.options || {};
            if (!opts.organism_contains) {
                if (/(^|\b)(human|homo sapiens)(\b|$)/i.test(p.hypothesis))
                    opts.organism_contains = 'Homo sapiens';
                else if (/(mus musculus|mouse)/i.test(p.hypothesis))
                    opts.organism_contains = 'Mus musculus';
                else if (/(rattus norvegicus|rat)/i.test(p.hypothesis))
                    opts.organism_contains = 'Rattus norvegicus';
                else {
                    // crude binomial detector: Genus species
                    const m = p.hypothesis.match(/([A-Z][a-z]+\s+[a-z]{3,})/);
                    if (m)
                        opts.organism_contains = m[1];
                }
            }
        }
        catch { }
...
tools.push({
    ...
    handler: async (args) => {
        const rows = await searchTrialsCtgov(args);
    ...
    try {
        ...
        if (method === 'tools/call') {
            ...
            try {
                ...
                return res.json(ok(id, { content: result }));
            }
            catch (e) {
                return res.json(err(id, -32000, e?.message || 'Tool error'));
            }
        }
