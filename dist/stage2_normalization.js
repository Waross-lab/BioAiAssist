...
export async function resolveCompoundIdentity(q) {
    const base = 'https://pubchem.ncbi.nlm.nih.gov/rest/pug';
    let path = '';
    if (q.inchikey)
        path = `/compound/inchikey/${encodeURIComponent(q.inchikey)}/property/InChIKey,CanonicalSMILES/JSON`;
    else if (q.cid)
        path = `/compound/cid/${encodeURIComponent(q.cid)}/property/InChIKey,CanonicalSMILES/JSON`;
    else if (q.smiles)
        path = `/compound/smiles/${encodeURIComponent(q.smiles)}/property/InChIKey,CanonicalSMILES/JSON`;
    else if (q.name)
        path = `/compound/name/${encodeURIComponent(q.name)}/property/InChIKey,CanonicalSMILES/JSON`;
    else
        return null;
    ...
    const prop = first(j?.PropertyTable?.Properties);
    if (!prop)
        return null;
...
export function chemblTargetToUniProt(target) {
    // Expecting shape similar to ChEMBL API: target_components: [{ target_component: ..., target_component_synonyms: ..., accession }]
    const comp = first(target?.target_components) || first(target?.components);
 ...
 * Normalizers
 * ========================= */
export function normalizePubChemProps(input, nameHint) {
    const out = [];
    for (const r of (input || [])) {
        const inchikey = (r.InChIKey || r.inchikey || undefined);
        ...
        out.push({
            ...
            formula: (r.MolecularFormula || r.Formula),
...
export function normalizeChemblTargets(rows) {
    const out = [];
    for (const t of (rows || [])) {
...
export function normalizeChemblActivities(rows, targetLookup) {
    const out = [];
    for (const a of (rows || [])) {
...
export function normalizeUniProtSearch(rows) {
    const out = [];
    for (const r of (rows || [])) {
        out.push({
...
export function normalizeLiteratureEntries(pubmedEsearch, europepmc, openalex) {
    const out = [];
    // PubMed
    const es = pubmedEsearch?.esearchresult || pubmedEsearch;
    const pmids = (es?.idlist || es?.idList || []).map((x) => String(x));
    for (const pmid of pmids)
        out.push({ key: `PMID:${pmid}`, pmid, source: 'pubmed' });
    // EuropePMC
...
async function augmentChemblTargetsWithUniProt(targets) {
    const out = [...targets];
    ...
    for (const id of uniqueMissing) {
        ...
        const comp = first(j?.target_components) || first(j?.components);
        const acc = comp?.accession || comp?.uniprot_accession || comp?.component_accession;
        if (acc) {
            for (const t of out) {
...
async function enrichCompoundsIdentity(compounds) {
    const out = [...compounds];
    for (const c of out) {
        if (!c.inchikey && c.cid) {
...
export async function normalizeEntities(input) {
    ...
    for (const t of targets) {
        if (t.chembl_target_id && t.uniprot)
            tLookup[t.chembl_target_id] = t.uniprot;
    }
