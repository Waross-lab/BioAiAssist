...
// scripts/test_encode.mjs
// Usage: node scripts/test_encode.mjs "ATAC-seq in Homo sapiens K562"
...
function detectAssay(text) {
  if (!text) return null;
  const m = text.match(/(chip-?seq|atac-?seq|rna-?seq|dnase-?seq|hi-?c)/i);
  return m ? m[1].replace(/-/g,"-") : null;
}

function detectCellHint(text) {
  if (!text) return null;
  // naive pull of uppercase tokens like K562, GM12878, HEK293, etc.
  const m = text.match(/\b([A-Z]{2,}\d{2,}|HEK293|HeLa|K562|GM12878|HCT116|A549|HepG2|U2OS)\b/);
  return m ? m[1] : null;
}
...
async function main() {
  ...
  // ENCODE search uses biosample_ontology.term_name; try that if we have a cell hint
  ...
  const rows = Array.isArray(data?.["@graph"]) ? data["@graph"] : [];

  // Summarize by cell type and audits
  const byCell = new Map();
  const sevOrder = ["ERROR","NOT_COMPLIANT","WARNING","DCC_ACTION","INTERNAL_ACTION"];
  function getCell(r) {
    const t = r?.biosample_ontology?.term_name || r?.biosample_term_name || "";
    if (t) return t;
    const s = r?.biosample_summary || "";
    const m = s.match(/^([^,;]+)[,;]?/);
  ...
  function getSevCounts(r) {
    ...
    for (const sev of Object.keys(a)) {
      if (!counts.hasOwnProperty(sev)) continue;
  ...
  for (const r of rows) {
    ...
    const c = byCell.get(cellType) || { n:0, sev:{ ERROR:0, WARNING:0, NOT_COMPLIANT:0, DCC_ACTION:0, INTERNAL_ACTION:0 } };
    c.n += 1;
    const sc = getSevCounts(r);
    for (const k of Object.keys(c.sev)) c.sev[k] += sc[k];
    byCell.set(cellType, c);
  }
  ...
  // Show a few "when it goes bad" examples
  ...
  if (bad.length) {
    ...
    for (const r of bad) {
      ...
      const cellType = getCell(r);
      const errs = (r?.audit?.ERROR || []).map(x=>x.category).join(", ");
      const ncs = (r?.audit?.NOT_COMPLIANT || []).map(x=>x.category).join(", ");
